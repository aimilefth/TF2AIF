...
This module defines the GpuServer class, which inherits from the BaseExperimentServer class defined in the experiment_server module. The GpuServer class represents a GPU-based implementation of the experiment server, optimized for running ONNX Runtime models.
...
class GpuServer(experiment_server.BaseExperimentServer):
    def __init__(self, logger):
        ...
        self.sess = None
        # GPU-specific configuration setup (ONNX)
    ...
    def init_kernel(self):
        ...
        if(self.server_configs['PRECISION'].lower() == 'fp32'): # Configuration for fp32 precision
        ...
        elif(self.server_configs['PRECISION'].lower() == 'fp16'): # Configuration for fp16 precision
            self.server_configs['providers'] = [
            ('TensorrtExecutionProvider',{
             'device_id' : 0,
        ...
        elif(self.server_configs['PRECISION'].lower() == 'int8'): # Configuration for int8 precision
            self.server_configs['providers'] = [
            ('TensorrtExecutionProvider',{
             'device_id' : 0,
             'trt_fp16_enable' : False,
            ...
            raise AssertionError ('Incorrect precision environmental variable, got {}'.format(self.server_configs['PRECISION']))
