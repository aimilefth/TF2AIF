...
# Check for at least one .onnx file in the current directory
onnx_files=$(find . -name "*.onnx" -type f)
if [[ -z "$onnx_files" ]]; then
  echo "No .onnx files found."
  exit 1
fi

# Check for at least one .flatbuffers file in the current directory
flatbuffers_files=$(find . -name "*.flatbuffers" -type f)
if [[ -z "$flatbuffers_files" ]]; then
  echo "No .flatbuffers files found."
  exit 1
fi
