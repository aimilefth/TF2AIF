...
# Check for at least one directory that contains an 'assets' directory, a 'variables' directory, and a '.pb' file
required_dir=$(find . -type d -exec test -d '{}/assets' -a -d '{}/variables' -a -f '{}/saved_model.pb' \; -print -quit)

if [[ -z "$required_dir" ]]; then
  echo "No directory found that contains an 'assets' directory, a 'variables' directory, and a 'saved_model.pb' file."
  exit 1
fi
...
docker buildx build -f ${SRC_COMPOSER_DIR}/${NAME}/Dockerfile.${NAME,,} --platform linux/arm64 $build_args --tag ${REPO}:${LABEL}_${NAME,,} --push .
status=$?
if [ $status -eq 0 ]; then
